...
radioButtons.forEach(button => {
    button.addEventListener('click', () => {
        ...
        let newIndex = button.getAttribute('id').substring(button.getAttribute('id').length - 1, button.getAttribute('id').length);
...
timeButtons.forEach(button => {
    button.addEventListener('click', () => {
        const activeTime = times.querySelector("[data-active]");
        const newIndex = button.getAttribute('id').substring(button.getAttribute('id').length - 1, button.getAttribute('id').length);
